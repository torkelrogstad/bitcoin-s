...
trait RawHeadersMessageSerializer extends RawBitcoinSerializer[HeadersMessage] {
  ...
  private def parseBlockHeaders(bytes: ByteVector, compactSizeUInt: CompactSizeUInt): Seq[BlockHeader] = {
    @tailrec
    def loop(remainingBytes: ByteVector, remainingHeaders: Long, accum: List[BlockHeader]): Seq[BlockHeader] = {
