...
import org.bitcoins.node.constant.{Constants, DbConfig}
...
sealed abstract class BlockHeaderDAO extends CRUDActor[BlockHeader, DoubleSha256Digest] {
  ...
  private def handleBlockHeaderDAORequest(
    ...
    case createAllMsg: BlockHeaderDAO.CreateAll =>
      val createAllHeaders = createAll(createAllMsg.blockHeaders)
        .map(BlockHeaderDAO.CreateAllReply(_))(context.dispatcher)
      sendToParent(createAllHeaders)
    case readMsg: BlockHeaderDAO.Read =>
      val readHeader = read(readMsg.hash)
      val reply =
        readHeader.map(BlockHeaderDAO.ReadReply(_))(context.dispatcher)
      sendToParent(reply)
    ...
    case BlockHeaderDAO.LastSavedHeader =>
      ...
      val reply = lastSavedHeader.map(headers =>
        BlockHeaderDAO.LastSavedHeaderReply(headers))(context.dispatcher)
      reply.map(r => logger.error("Last saved header reply: " + r))(
        context.dispatcher)
      sendToParent(reply)
    case BlockHeaderDAO.GetAtHeight(height) =>
      val result = getAtHeight(height)
      val reply = result.map(h => BlockHeaderDAO.GetAtHeightReply(h._1, h._2))(
        context.dispatcher)
      sendToParent(reply)
    ...
    case BlockHeaderDAO.MaxHeight =>
      ...
      val reply =
        result.map(BlockHeaderDAO.MaxHeightReply(_))(context.dispatcher)
      sendToParent(reply)
  }

  /** Sends a message to our parent actor */
  private def sendToParent(returnMsg: Future[Any]): Unit =
    returnMsg.onComplete {
      case Success(msg) =>
        context.parent ! msg
  ...
  def find(blockHeader: BlockHeader): Query[Table[_], BlockHeader, Seq] =
    findByPrimaryKey(blockHeader.hash)

  def findByPrimaryKey(
      hash: DoubleSha256Digest): Query[Table[_], BlockHeader, Seq] = {
  ...
  def findHeight(
    ...
    val b: Future[Option[(Long, BlockHeader)]] =
      database.run(query).map(_.headOption)(context.dispatcher)
    b
  }
...
object BlockHeaderDAO {
  ...
  case class CreateReply(blockHeader: BlockHeader)
      extends BlockHeaderDAOMessageReplies

  /** The message to create all [[BlockHeader]]s in our persistent storage */
  case class CreateAll(blockHeaders: Seq[BlockHeader])
      extends BlockHeaderDAORequest

  /** The reply to the message [[CreateAll]] */
  case class CreateAllReply(headers: Seq[BlockHeader])
      extends BlockHeaderDAOMessageReplies
  ...
  case class ReadReply(hash: Option[BlockHeader])
      extends BlockHeaderDAOMessageReplies
  ...
  case class DeleteReply(blockHeader: Option[BlockHeader])
      extends BlockHeaderDAOMessageReplies
  ...
  case class LastSavedHeaderReply(headers: Seq[BlockHeader])
      extends BlockHeaderDAOMessageReplies
  ...
  case class GetAtHeightReply(height: Long, headers: Seq[BlockHeader])
      extends BlockHeaderDAOMessageReplies
  ...
  /** Returns the height of the [[BlockHeader]], this is a reply to the [[FindHeight]] message
    ...
    * competing chains
    */
  case class FoundHeight(headerAtHeight: Option[(Long, BlockHeader)])
      extends BlockHeaderDAOMessageReplies
  ...
  def props(dbConfig: DbConfig): Props =
    Props(classOf[BlockHeaderDAOImpl], dbConfig)
  ...
  def apply(dbConfig: DbConfig): ActorRef =
    BlockHeaderDAO(Constants.actorSystem, dbConfig)

  def apply: ActorRef =
    BlockHeaderDAO(Constants.actorSystem, Constants.dbConfig)
}
