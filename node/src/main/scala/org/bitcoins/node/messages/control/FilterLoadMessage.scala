...
object FilterLoadMessage extends Factory[FilterLoadMessage] {
  ...
  override def fromBytes(bytes: ByteVector): FilterLoadMessage = RawFilterLoadMessageSerializer.read(bytes)

  def apply(filterSize: CompactSizeUInt, filter: ByteVector, hashFuncs: UInt32, tweak: UInt32, flags: BloomFlag): FilterLoadMessage = {
