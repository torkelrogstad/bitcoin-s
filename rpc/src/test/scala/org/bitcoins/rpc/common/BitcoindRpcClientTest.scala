...
import org.bitcoins.rpc.{BitcoindRpcTestUtil, RpcUtil, TestUtil}
...
class BitcoindRpcClientTest
  ...
  it should "be able to invalidate a block" in {
    otherClient.getNewAddress.flatMap { address =>
      TestUtil.fundMemPoolTransaction(client, address, Bitcoins(3)).flatMap {
        txid =>
  ...
  it should "be able to get a new raw change address" in {
    client.getRawChangeAddress
      .map(_ => client.getRawChangeAddress(AddressType.Legacy))
      .map(_ => client.getRawChangeAddress(AddressType.Bech32))
      .map(_ => client.getRawChangeAddress(AddressType.P2SHSegwit))
      .map(_ => succeed)
  }
  ...
  it should "be able to add and remove a node" in {
    otherClient
      .addNode(walletClient.getDaemon.uri, AddNodeArgument.Add)
      .flatMap { _ =>
        ...
        otherClient.getAddedNodeInfo(walletClient.getDaemon.uri).flatMap {
          info =>
            ...
            otherClient
              .addNode(walletClient.getDaemon.uri, AddNodeArgument.Remove)
              .flatMap { _ =>
