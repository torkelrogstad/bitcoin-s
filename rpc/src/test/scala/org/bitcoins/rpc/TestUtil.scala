...
trait TestUtil extends BitcoinSLogger {
  ...
  /**
    ...
    */
  def startServers(servers: BitcoindRpcClient*)(
  ...
  /**
    ...
    */
  def stopServers(servers: BitcoindRpcClient*)(
      implicit system: ActorSystem): Unit =
  ...
  def awaitSynced(
    ...
    implicit val ec: ExecutionContext = system.dispatcher
    RpcUtil.awaitCondition(() =>
                             Await.result(client1.getBlockCount.flatMap {
                               count1 =>
  ...
  def createNodePair(
    ...
    client1
      .addNode(client2.getDaemon.uri, AddNodeArgument.Add)
      .map(_ => {
  ...
  def deleteNodePair(client1: BitcoindRpcClient, client2: BitcoindRpcClient)(
      implicit actorSystem: ActorSystem): Unit = stopServers(client1, client2)
