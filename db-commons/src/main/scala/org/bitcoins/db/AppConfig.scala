...
trait AppConfig extends BitcoinSLogger {
  ...
  private def createDbFileIfDNE(): Unit = {
    ...
    if (!Files.exists(dbPath)) {
      logger.info(s"Creating database directory=$dbPath")
  ...
  lazy protected val config: Config = {
    ...
    logger.info(s"Unresolved bitcoin-s config:")
    logger.info(unresolvedConfig.getConfig("bitcoin-s").asReadableJson)
    ...
    logger.info(config.asReadableJson)
