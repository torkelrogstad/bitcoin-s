...
sealed abstract class BaseECKey extends NetworkElement with Sign {
  ...
  private def sign(
    ...
    val signature = Secp256k1.sign(dataToSign.toArray, signingKey.bytes.toArray)
...
sealed abstract class ECPrivateKey extends BaseECKey {
  ...
  def publicKey: ECPublicKey = {
    val pubKeyBytes: Array[Byte] =
      Secp256k1.computePubkey(bytes.toArray, isCompressed)
    require(
      Secp256k1.isValidPubKey(pubKeyBytes),
...
object ECPrivateKey extends Factory[ECPrivateKey] {
  ...
  private case class ECPrivateKeyImpl(
    ...
    require(Secp256k1.secKeyVerify(bytes.toArray),
...
sealed abstract class ECPublicKey extends BaseECKey {
  ...
  def verify(data: ByteVector, signature: ECDigitalSignature): Boolean = {
    Secp256k1.verify(data.toArray, signature.bytes.toArray, bytes.toArray)
  }
  ...
  def decompressed: ECPublicKey = {
    if (isCompressed) {
      val decompressed = Secp256k1.decompress(bytes.toArray)
...
object ECPublicKey extends Factory[ECPublicKey] {
  ...
  def isFullyValid(bytes: ByteVector): Boolean =
    Try(Secp256k1.isValidPubKey(bytes.toArray))
